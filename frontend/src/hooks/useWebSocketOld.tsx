...
function useWebSocket(
    ...
    const connect = useCallback(() => {
        if (!token) {
            logger.info("Token为空，跳过Socket.IO连接。");
            return;
        }

        logger.info('连接到 Socket.IO 服务器:', apiUrl);
        ...
        socket.on('message', (data: any) => {
            logger.info('收到Socket.IO消息:', data);
            onMessageRef.current(data);
        });

        socket.on('chat_history', (data: any) => {
            logger.info('收到聊天历史:', data);
            onMessageRef.current(data);
        });

        socket.on('disconnect', (reason: string) => {
            logger.info('Socket.IO已断开:', reason);
            ...
            if (reason !== 'io client disconnect') { // 非正常断开
                if (retryCountRef.current < MAX_RETRIES) {
                    ...
                    logger.info(`将在${retryDelay / 1000}秒后尝试重新连接...`);
                    reconnectTimeoutRef.current = setTimeout(() => {
                        retryCountRef.current += 1;
                        connect();
                    }, retryDelay);
                } else {
                    logger.info('已达到最大重试次数，不再尝试重新连接。');
                }
            }
        });
    ...
    useEffect(() => {
        logger.info('useWebSocket useEffect', token);
        if (!token) return;
    ...
    const sendMessage = useCallback((event: string, msg: any) => {
        logger.info(`尝试发送消息 (${event}):`, msg);
        if (socketRef.current && socketRef.current.connected) {
            logger.info(`消息已发送 (${event}):`, msg);
            socketRef.current.emit(event, msg);
        } else {
            logger.info('Socket.IO 是否连接',socketRef.current.connected);
            logger.info('Socket.IO 未连接，无法发送消息。');
        }
    }, []);
