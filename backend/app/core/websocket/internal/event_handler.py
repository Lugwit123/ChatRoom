...
class WebSocketEventHandler(IWebSocketEventHandler):
    """WebSocket事件处理器
    ...
    def __init__(self, connection_manager: IConnectionManager, room_manager: IRoomManager):
        """初始化事件处理器
    ...
    async def on_connect(self, sid: str, environ: dict, auth: Optional[dict] = None) -> None:
        ...
        try:
            ...
            # 添加连接
            await self._connection_manager.add_connection(sid, auth['user_id'], device_id, ip_address)
            lprint(f"新连接已建立: sid={sid}")
