...
本文件实现了群组消息的辅助工具功能，主要用于消息的导出、备份和恢复等操作。

GroupMessageUtils类的主要功能:
1. 消息导出:
   - export_messages(): 导出群组消息
   - export_to_json(): 导出为JSON格式
   - export_to_csv(): 导出为CSV格式
   - export_to_html(): 导出为HTML格式

2. 消息备份:
   - backup_messages(): 备份群组消息
   - create_backup(): 创建备份
   - list_backups(): 列出所有备份
   - delete_backup(): 删除备份

3. 消息恢复:
   - restore_messages(): 从备份恢复消息
   - validate_backup(): 验证备份文件
   - restore_from_file(): 从文件恢复

4. 消息转换:
   - convert_format(): 转换消息格式
